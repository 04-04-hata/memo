...
前までコンストラクタが定義されていないHeroクラスはnew Hero();で生成できたのに、
引数ありコンストラクタを定義しただけで同様の事↑が不可能になったのは何故か<br>

実はJavaでは、全てのクラスはインスタンス化に際して必ず何らかのコンストラクタを実行することになっている為、
本来全てのクラスは最低でも一つ以上のコンストラクタ定義を持っていなければならない。<br>
コンストラクタが一つも定義されていないクラスは許されない<br>

コンストラクタの特例<br>

クラスに一つもコンストラクタが定義されていない場合に限って、
引数なし、処理内容なしのコンストラクタ(デフォルトコンストラクタ)が
コンパイル時に自動的に追加される。<br>

これまで示してきたHeroクラスは、この特例によって引数なしコンストラクタがこっそり自動的に定義された為、
new Hero();によるインスタンス化が可能だった。<br>
だが、新たに引数を一つ含むコンストラクタ「public Hero(String name)」を
定義した時点でこの特例は適用されなくなり、new Hero();という記述によるインスタンスの生成は出来なくなった。<br>

他のコンストラクタを呼び出す<br>

コンストラクタを呼び出せるのはJVMだけで開発者が直接呼ぶことは許されていないが、
以下の構文↓によってJVMにコンストラクタの起動を依頼することが可能。<br>

同一クラスないの別コンストラクタの呼び出しをJVMに依頼する<br>
this(引数); ※コンストラクタの先頭に記述する必要がある<br>
...
※this()はthisと見た目が似ているが、何の関係もない別物と考えたほうがいい。<br>
this.メンバ名のthisは自分自身のインスタンスを表すもの。<br>
this(引数)のthis()は同一クラスの別コンストラクタを呼び出すためのもの。<br>
...
- クラス型は、フィールドの方や、メソッドの引数・戻り値の型としても利用できる
<br>

コンストラクタ<br>
- クラスメイト同一名称で、戻り値の方が明記されていないメソッドはコンストラクタとして扱われる
- コンストラクタは、newによるインスタンス化の直後に自動的に実行される
- 引数を持つコンストラクタを定義すると、newをする際に引数を指定してコンストラクタを実行させることができる
- コンストラクタはオーバーロードにより複数定義出来る
- クラスにコンストラクタ定義が一つもない場合に限って、コンパイラが引数なし、処理内容なしのデフォルトコンストラクタを自動的に定義してくれる
- this()を用いれば同一クラス内の別コンストラクタの呼び出しをJVMに依頼する事が出来る
